...
class MyUserCreationForm(UserCreationForm):  # 增加用户表单重新定义，继承自UserCreationForm
    ...
    def __init__(self, *args, **kwargs):
        ...
        # 其实这个name字段可以不用设定required，因为在models中的MyUser类中已经设定了blank=False，但email字段在系统自带User的models中已经设定为
        self.fields['name']self.required = True
        # email = models.EmailField(_('email address'),
        # blank=True)，除非直接改源码的django（不建议这么做），不然还是自定义一个表单做一下继承吧。
...
class CustomUserAdmin(UserAdmin):
    ...
    def def __init__(self, args):
        ...
        self.list_display = ('username', 'name', 'email',
        ...
        if cond:
            ...
            not request.user.is_superuser:  # 非super用户不能设定编辑是否为super用户
            self.fieldsets = ((None, {'fields': ('username', 'password',)}),
                              # _
                              # 将('')里的内容国际化,这样可以让admin里的文字自动随着LANGUAGE_CODE切换中英文
                              (_('Personal info'), {
                               'fields': ('name', 'email')}),
                              (_('Permissions'), {
